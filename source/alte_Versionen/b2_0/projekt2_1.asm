; **********************************************************
; * projekt_beta_1_9_1.asm - Playstation Controller an PIC16F84A *
; **********************************************************
;
...
; mit der Adresse H'20' aufsteigend vergeben.
	...
	CBLOCK	H'20'
		...
		byte_anz		;wird Byte 0 oder 1 uebertragen....
...
kMODUS_adr	EQU	B'00001001' ; 0x09 Modusbefehlsadresse
m_MODUS		EQU	B'00000000' ; 0x00 MAX Digit Kodierungsmodus		->keine Kodierung<-

kD_MODUS_adr	EQU	B'00001001' ; 0x09 Digit Kodierungsmodus Adresse
m_D_MODUS	EQU	B'00000000' ; 0x00 MAX Digit Kodierungsmodus		->keine Kodierung<-

;MAX Digit Helligkeit
kHELLIGKEIT_adr	EQU	B'00001010' ; 0x0A MAX Helligkeitssteuerung Adresse
m_HELLIGKEIT	EQU	B'00001111' ; 0x15 MAX Helligkeitssteuerung		->Digit 31 / 32 (max ON)<-

;MAX Digit Anzahl
kDIGITS_adr	EQU	B'00001011' ; 0x0B Digitsanzahl Adresse
m_DIGITS		EQU	B'00000001' ; 0x01 Anzahl der aktivierten Digits 	->Digit 0 und 1<-

;MAX Shutdown
kSTART_adr	EQU	B'00001100' ; 0x0C Startbefehlsadresse
m_START		EQU	B'00000001' ; 0x01 MAX Shutdownbefehl 		->Normal Operation<-

kSHUTDOWN_adr	EQU	B'00001100' ; 0x0C SHUTDOWN Adresse
m_SHUTDOWN	EQU	B'00000001' ; 0x01 MAX Shutdownbefehl 		->Normal Operation<-

;MAX Test
kTEST_adr	EQU	B'00001111' ; 0x0F Testbefehlsadresse
m_TEST		EQU	B'00000000' ; 0x00 MAX Testmodus 			->KEIN TESTMODUS<-

;zur Digitauswahl
;Könnte auch mit einem Zaehler gemacht werden, aber ich möchte auswählen
;wo welches Register angezeigt werden soll
;später mach ich es mit TABELLE!!!!!!!!!!!!!
...
; gang eine '0' setzt das Bit als Ausgang.
	...
	BSF	a_MAX_LOAD	;MAX-Baustein soll nicht im Übertragungsmodus sein
;*****************
;* Hauptprogramm *
;*****************
...
;###################################
	MOVLW	kTYPE_adr	; Digit Adresse senden
	...
	BSF	a_MAX_LOAD
         ...
         ;CALL	UP_wait_05s
;###################################
	MOVLW	kSTATUS_adr	; Digit Adresse senden
	;MOVWF	PORTB		;-------DEBUGGING--------
	;CALL	UP_wait_05s	;-------DEBUGGING--------
         CALL	UP_MAX_send	; 
         ;-
...
;m_TEST		EQU	B'00000000' ; 0x00 MAX Testmodus 			->KEIN TESTMODUS<-
	...
	MOVLW	kDIGITS_adr	; erst Adresse
	;MOVWF	PORTB		;-------DEBUGGING--------
	;CALL	UP_wait_05s	;-------DEBUGGING--------		
	CALL	UP_MAX_send	; 	0x0B
	;#########	         ;
	MOVLW	m_DIGITS		; dann die Anzahl der Digits
	;MOVWF	PORTB		;-------DEBUGGING--------
	;CALL	UP_wait_05s	;-------DEBUGGING--------		
	...
	;###########################	Shutdownmodus senden	
				; 
	MOVLW	kSHUTDOWN_adr	; erst Adresse
	;MOVWF	PORTB		;-------DEBUGGING--------
	;CALL	UP_wait_05s	;-------DEBUGGING--------
         CALL	UP_MAX_send	; 	0x0C
         ;#########		; dann der Befehl selbst senden
	MOVLW	m_SHUTDOWN	; 	0x01 (Normal Operation)
	;MOVWF	PORTB		;-------DEBUGGING--------
	;CALL	UP_wait_05s	;-------DEBUGGING--------
         CALL	UP_MAX_send	;
	...
	;###########################	Testmodus senden	
				; 
	MOVLW	kTEST_adr	; erst Adresse
	;MOVWF	PORTB		;-------DEBUGGING--------
	;CALL	UP_wait_05s	;-------DEBUGGING--------
         CALL	UP_MAX_send	; 	0x0C
         ;#########		; dann der Befehl selbst senden
	MOVLW	m_TEST		; 	0x01 (Normal Operation)
	;MOVWF	PORTB		;-------DEBUGGING--------
	;CALL	UP_wait_05s	;-------DEBUGGING--------
         CALL	UP_MAX_send	;
	...
	MOVLW	kMODUS_adr	; erst Adresse
	;MOVWF	PORTB		;-------DEBUGGING--------
	;CALL	UP_wait_05s	;-------DEBUGGING--------	
         CALL	UP_MAX_send	;	0x09 
         ;#########		; dann der Befehl selbst
         MOVLW	m_MODUS		;	0x00 (keine Kodierung der Digits)
         ;MOVWF	PORTB		;-------DEBUGGING--------
         ;CALL	UP_wait_05s	;-------DEBUGGING--------
         CALL	UP_MAX_send	;
	...
	MOVLW	kHELLIGKEIT_adr	; erst Adresse
	;MOVWF	PORTB		;-------DEBUGGING--------
	;CALL	UP_wait_05s	;-------DEBUGGING--------
         CALL	UP_MAX_send	; 	0x0A
         ;#########		; dann die Helligkeitseinstellung selbst
         MOVLW	m_HELLIGKEIT	;	0x15 (31/32 an)
	;MOVWF	PORTB		;-------DEBUGGING--------
	;CALL	UP_wait_05s	;-------DEBUGGING--------		
         CALL	UP_MAX_send	; 
...
;
tab_MAX				; Tabellen länge: 22; -> zaehler max. 23!
	ORG	0x300		; Tabelle in Speicherblock 3

	RETLW	kDIGITS_adr	; Anzahl der genutzten Digits
	RETLW	m_DIGITS		; 

	RETLW	kSHUTDOWN_adr	; Normalmodus oder Ausgeschaltet
	RETLW	m_SHUTDOWN	; 

	RETLW	kTEST_adr	; Normalmodus oder Displaytest
	RETLW	m_TEST		; 
	
	RETLW	kD_MODUS_adr	; Dekodierungsmodus 
	RETLW	m_D_MODUS	; 

	RETLW	kHELLIGKEIT_adr	; Helligkeitssteuerung der LED's (WAZUP ???)
	RETLW	m_HELLIGKEIT	;
 
	;RETLW	kTYPE_adr	; Controller Type
	;RETLW	gTYPE		; Type - Digit-Adresse
	
	;RETLW	kSTATUS_adr	; Controller Status
	;RETLW	gSTATUS		; Status - Digit-Adresse

	RETLW	kLEFT_adr	; Buttons LINKS
	RETLW	gLEFT		; LINKS - Digit-Adresse

	RETLW	kRIGHT_adr	; Buttons RECHTS
	RETLW	gRIGHT		; RECHTS - Digit-Adresse

	RETLW	kRJoyX_adr	; Joystick rechts x-Achse
	RETLW	gRJoyX		; Joystick rechts x-Achse - Digit-Adresse

	RETLW	kRJoyY_adr	; Joystick rechts y-Achse
	RETLW	gRJoyY		; Joystick rechts y-Achse - Digit-Adresse

	RETLW	kLJoyX_adr	; Joystick links  x-Achse
	RETLW	gLJoyX		; Joystick links x-Achse - Digit-Adresse

	RETLW	kLJoyY_adr	; Joystick links	y-Achse
	RETLW	gLJoyY		; Joystick links y-Achse - Digit-Adresse
;
;zur Initialisierung:
