...
;###################################
	MOVF	kTYPE_adr,W	; Digit Adresse senden
	MOVWF	sMAX
         CALL	UP_MAX_send	;  
	;-
	;MOVF	gTYPE,W		;
	...
	BSF	a_MAX_LOAD
         ...
         ;CALL	UP_wait_05s
;###################################
	MOVF	kSTATUS_adr,W	; Digit Adresse senden
	MOVWF	sMAX
         CALL	UP_MAX_send	; 
         ;-
         ;MOVF	gSTATUS,W	;
...
;m_TEST		EQU	B'00000000' ; 0x00 MAX Testmodus 			->KEIN TESTMODUS<-
	...
	MOVF	kDIGITS_adr,W	; erst Adresse
	MOVWF	sMAX		
	...
	;###########################	Shutdownmodus senden	
				; 
	MOVF	kSHUTDOWN_adr,W	; erst Adresse
	MOVWF	sMAX
	...
	MOVF	kMODUS_adr,W	; erst Adresse
	MOVWF	sMAX	
         CALL	UP_MAX_send	;	0x09 
         ;#########		; dann der Befehl selbst
         MOVF	m_MODUS,W	;	0x00 (keine Kodierung der Digits)
         MOVWF	sMAX
         CALL	UP_MAX_send	;
	...
	MOVF	kHELLIGKEIT_adr,W	; erst Adresse
	MOVWF	sMAX
         CALL	UP_MAX_send	; 	0x0A
         ;#########		; dann die Helligkeitseinstellung selbst
         MOVF	m_HELLIGKEIT,W	;	0x15 (31/32 an)
	MOVWF	sMAX		
         CALL	UP_MAX_send	; 
