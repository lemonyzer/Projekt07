...
;#MAN KANN AUCH
;MOWLW [ANZAHL der Tabelleneinträge]
;#machen und dann später auf
;#NULL überprüfen mit
;MOWF	zaehler,F
;-----------------------------------------------------------
;Dann wird die Tabelle aber von hinten nach vorne angezeigt!
;-----------------------------------------------------------
         MOVLW	D'1'
	MOVWF	zaehler		;Zaehler auf 1 setzten
	
Show_1	CLRF	PORTB		;PORTB (Anzeige löschen)
         MOVFW	zaehler		;Zaehler ins Wreg. um mit PCL zu addieren
	...
	SUBWF	zaehler,F	;subtrahieren um
...
tab_Info	;zaehler maximal bis 3
	ADDWF	PCL,F		;   
	DT	gTYPE,gSTATUS	; Controller Type und der Status
	
;#########################     L/R Buttons Tabelle     #########################

tab_Btns	;zaehler maximal bis 3
	ADDWF	PCL,F		; Anzeigen der gedrückten (0)  
	DT	gLEFT,gRIGHT	; und nicht gedrückten (1) Btns [LINKS,RECHTS]
...
tab_Joy	;zaehler maximal bis 5
	...
	DT	B'00110011',B'11001100'
	DT	B'10000000',B'00000001'
;#########################         ALL Tabelle        ##########################

tab_All	;zaehler maximal bis 9
	ADDWF	PCL,F		;   
	DT	gTYPE,gSTATUS	; Controller Type und der Status
	DT	gLEFT,gRIGHT	; Buttons [LINKS,RECHTS]
	DT	gRJoyX,gRJoyY	; Joystick rechts [x-Achse,y-Achse]
	DT	gLJoyX,gLJoyY	; Joystick links  [x-Achse,y-Achse]
