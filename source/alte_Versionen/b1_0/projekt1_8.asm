...
;m_DIGITS	EQU	B'00000001' ; Anzahl der aktivierten Digits
	...
	MOVF	kHELLIGKEIT_adr,W	;Helligkeits Adresse laden
         CALL	UP_MAX_send	;sende 2. 8 Bits
         ;#########
         MOVF	m_HELLIGKEIT,W	;Helligkeitsbefehl laden		
         CALL	UP_MAX_send	;sende 1. 8 Bits
         
         BSF	a_MAX_LOAD	; Nach 2 Byte (1 word) muss LOAD auf HIGH

;	MOVF	kDIGITS_adr,W	;DIGITs Adresse laden		
;	CALL	UP_MAX_send	;sende 2. 8 Bits
;	;#########	         
;	MOVF	m_DIGITS,W	;Benutzte Digits laden		
;	CALL	UP_MAX_send	;sende 1. 8 Bits
;	
;	BSF	a_MAX_LOAD	; Nach 2 Byte (1 word) muss LOAD auf HIGH
...
MAX2_s1	RLF	sMAX_B1		;MSB zuerst...
	BTFSC	STATUS,C
	GOTO	MAX2_send_1	;Übertrage 1
	BCF	a_MAX_DATA	;Übertrage 0	
	
MAX2_s2	BCF	a_MAX_CLOCK	;
	BSF	a_MAX_CLOCK	;Positive Flanke erzeugen
	BCF	a_MAX_CLOCK
	...
	GOTO	MAX2_s1
		        	  	;Schleifen ende
	;BSF	a_MAX_LOAD	;NACH der Übertragung von 2 Bytes auf LOW
	GOTO	MAX2_s_byte2	;Nächstes Byte übertragen
...
MAX2_s3	RLF	sMAX_B2		;MSB zuerst...
	BTFSC	STATUS,C
	GOTO	MAX2_byte2_send_1	;Übertrage 1
	BCF	a_MAX_DATA	;Übertrage 0	
	
MAX2_s4	BCF	a_MAX_CLOCK	;
	BSF	a_MAX_CLOCK	;Positive Flanke erzeugen
	BCF	a_MAX_CLOCK
