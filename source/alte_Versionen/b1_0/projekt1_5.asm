...
;zur Initialisierung:
kSTART		EQU	B'00001100' ; Startbefehlsadresse
kMODUS		EQU	B'00001001' ; Modusbefehlsadresse
;zur Digitauswahl
;Könnte auch mit einem Zaehler gemacht werden, aber ich möchte auswählen
;wo welches Register angezeigt werden soll
kTYPE_adr	EQU	B'00000000' ; Type - Digit-Adresse
kSTATUS_adr	EQU	B'00000001' ; Status - Digit-Adresse
kLEFT_adr	EQU	B'00000010' ; LINKS - Digit-Adresse
kRIGHT_adr	EQU	B'00000011' ; RECHTS - Digit-Adresse
kRJoyX_adr	EQU	B'00000100' ; Joystick rechts x-Achse - Digit-Adresse
kRJoyY_adr	EQU	B'00000101' ; Joystick rechts y-Achse - Digit-Adresse
kLJoyX_adr	EQU	B'00000110' ; Joystick links x-Achse - Digit-Adresse
kLJoyY_adr	EQU	B'00000111' ; Joystick links y-Achse - Digit-Adresse 
...
;###############################
#DEFINE	a_MAX_DATA	PORTB, 6		; MAX-7219-DATA	Ausgang
#DEFINE	a_MAX_CLOCK	PORTB, 7		; MAX-7219-CLOCK	Ausgang
;#####################################################################
...
UP_MAX
         ...
         MOVFW	gLEFT		;lade und ...
         CALL	UP_MAX_send	;sende 1. 8 Bits
         ;-
	MOVFW	kLEFT_adr	;lade und ...
         CALL	UP_MAX_send	;sende 2. 8 Bits (Auswahl des Anzeige-Digits)
         
         
         MOVFW	gRIGHT		;lade und ...
         CALL	UP_MAX_send	;sende 1. 8 Bits
         ;-
	MOVFW	kRIGHT_adr	;lade und ...
         CALL	UP_MAX_send	;sende 2. 8 Bits (Auswahl des Anzeige-Digits)
         
         
	MOVFW	gRJoyX		;lade und ...
	CALL	UP_MAX_send	;sende 1. 8 Bits
	;-
	MOVFW	kRJoyX_adr	;lade und ...
	CALL	UP_MAX_send	;sende 2. 8 Bits (Auswahl des Anzeige-Digits)
	
	
	MOVFW	gRJoyY		;lade und ...
	CALL	UP_MAX_send	;sende 1. 8 Bits
	;-
	MOVFW	kRJoyY_adr	;lade und ...
	CALL	UP_MAX_send	;sende 2. 8 Bits (Auswahl des Anzeige-Digits)
	
	
	MOVFW	gLJoyX		;lade und ...
	CALL	UP_MAX_send	;sende 1. 8 Bits
	;-
	MOVFW	kLJoyX_adr	;lade und ...
	CALL	UP_MAX_send	;sende 2. 8 Bits (Auswahl des Anzeige-Digits)
	
	MOVFW	gLJoyY		;lade und ...
	CALL	UP_MAX_send	;sende 1. 8 Bits
	;-
	MOVFW	kLJoyY_adr	;lade und ...
	CALL	UP_MAX_send	;sende 2. 8 Bits (Auswahl des Anzeige-Digits)
	
RETURN
;#####################################
UP_MAX_init
	MOVLW	D'00000001'	;Startbefehl laden		[01]
         CALL	UP_MAX_send	;sende 1. 8 Bits
         ;-
	MOVFW	D'00001100'	;Startbefehls Adresse laden	[xC]
         CALL	UP_MAX_send	;sende 2. 8 Bits
         
         MOVLW	D'00000000'	;Modusbefehl laden		[00]
         CALL	UP_MAX_send	;sende 1. 8 Bits
         ;-
	MOVFW	D'00001001'	;Modusbefehls Adresse laden	[x9]
         CALL	UP_MAX_send	;sende 2. 8 Bits
;#####################################
UP_MAX_send
	MOVWF	sMAX
...
MAX_s1	RRF	sMAX
	BTFSC	STATUS,C
	GOTO	MAX_send_1	;Übertrage 1
	BCF	a_MAX_DATA	;Übertrage 0	
